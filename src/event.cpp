...
void Event2::handle(State& s) {
    std::cout << util::time_to_string(get_time()) << " " << get_id() << " " << get_client() << std::endl;
    if (s.tables_[get_table_number()].current_owner != "") {
    ...
    for (int i = 1; i < s.tables_.size(); i++) { // not optimal
        if (s.tables_[i].current_owner == get_client()) {
            s.tables_[i].current_owner = ""; 
        }
    }

    s.tables_[get_table_number()].current_owner = get_client();
    return;
}
...
void Event4::handle(State& s) {
    ...
    for (int i = 1; i < s.tables_.size(); i++) {
        if (s.tables_[i].current_owner == get_client()) {
            if (s.queue_.size() > 0) {
                s.pool_.erase(s.tables_[i].current_owner);
                s.tables_[i].current_owner = "";
...
void Event12::handle(State& s) {
    std::cout << util::time_to_string(get_time()) << " " << get_id() << " " << get_client() << std::endl;
    s.tables_[get_table_number()].current_owner = s.queue_.front(); 
    s.queue_.pop();
    return;
}
